...
The workspace suite had **39 passed, 0 failed, 1 physical-hotkey fixture ignored**. The Release executable is `target/x86_64-pc-windows-msvc/release/LexWisp.exe` (31,833,088 bytes), SHA-256 `36686903894e0596795f208ee69ec15d69a41f284a80e2fe7d0be13b4192c0be`. The clean staged directory is `target/package/LexWisp-v0.1.1-windows-x64/` and contains exactly `LexWisp.exe`, `vcruntime140.dll`, `portable.flag`, `README.md`, and `THIRD-PARTY-NOTICES.txt`. The archive is `dist/LexWisp-v0.1.1-windows-x64.zip` (12,360,207 bytes), SHA-256 `deb33820054a07a7f9b2e0c36b4e575af0815714359616cf91fa207d2558ab63`.
...
The first GitHub Actions v0.1.1 attempt passed version validation, formatting, workspace check, and Clippy, then exposed two retention tests that used `Instant::now().checked_sub(2 hours)`. On a fresh runner with less than two hours of monotonic uptime, that fixture became `None` and both assertions failed. The tests now advance their pruning clock from a real finished timestamp, independent of machine uptime. All five local gates above passed again after this correction. The release job must pass again on the corrected tag before GitHub publication is considered complete.
