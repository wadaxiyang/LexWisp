...
This is the normative visual and interaction contract for the LexWisp product popup. The independent UI lab at `examples/lexwisp-ui-lab/src/lexwisp_main_window.rs` is its composition reference. The product adapts that interface to the real ChatController, storage, providers, settings, and Windows lifecycle. The lab remains independently scoped and is never a product dependency.

## 1. Product shape

LexWisp has one 680 × 640 DIP floating native window. Its pages are Chat, History, and Settings. The window may be hidden while the process continues through the tray and hotkey. Chat keeps the same conversation and in-flight run when hidden or reopened. There is no workspace presentation, second native panel, action chooser, or selected-text capture.

The global hotkey toggles the popup. The header, transcript/history/settings body, and composer form one continuous surface. Keep the visual center on the message and response, with Send as the primary action. Settings and History are accessible but visually quieter.
...
Views use `cx.theme()` semantic roles such as background, foreground, muted foreground, border, input, sidebar, popover, and group box. Do not put hex/rgb/hsl colors into view code. Keep one 4 px spacing rhythm and restrained rounded corners. Use one structural separator where a region boundary needs it. Do not create decorative cards around every adjacent group or use shadows on permanent panels. An overlay such as the conversation switcher may use a modest shadow.

## 3. Window and header

The popup has a 42 DIP header. It shows the current conversation title or page title, a conversation/back control, a drag region, New chat, Settings, Pin, and Hide. Icon-only buttons need accessible labels and tooltips. Pin is a real always-on-top native state. Hide goes through the surface controller and retains the warm window for the configured interval. Esc closes the model menu first, then the conversation switcher, then returns from History/Settings to Chat, then hides the popup.
...
The transcript occupies the flexible middle region. Empty Chat shows one short prompt to start. User content is a restrained tinted message; normal assistant content flows on a light ghost surface. Failed partial content uses the Kit destructive variant. Preserve selectable Markdown, answer copy, and individual fenced-code copy controls. Status appears only when useful: queued/generating/stopping, context warning, attachment error, save failure, or an unsaved result. Do not permanently render “Ready.”
...
The composer sits below the transcript as one contained surface: semantic group-box background, one input border, Textarea, attachment strip, model profile trigger, Send/Stop/Retry. The Textarea retains its entity and focus state. Enter sends a committed message; Shift+Enter inserts a newline; an IME candidate confirmation must not send. Disable Send for empty content or while the active conversation cannot send. Do not place inert switches for future tools in the composer.
...
History is a page inside the popup with search, Recent/Favorites filters, stable conversation rows, and a clear selected state. Favorites persist. Rename and Delete are real operations; Delete requires confirmation. Long lists should be paged or virtualized. A row uses transparent/quiet hover surfaces and truncated title rather than a bordered card.

## 6. Settings page

Settings is inside the same popup. Group controls by General, Provider and model, and History/data. General includes hotkey, Windows appearance, startup, local recording, and hidden-window retention. Provider controls include Base URL, model ID, optional Bearer key, stream, timeouts, proxy, context budget, temperature, and output limit. Test performs a real request. Save persists a valid provider and key through the designated services; the API key input is masked and cleared after saving. Backup creates a consistent local SQLite backup. All failures must be visible in the page.
